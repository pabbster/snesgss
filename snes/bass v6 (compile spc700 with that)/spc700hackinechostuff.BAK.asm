...
.setEffectEchoOn	//243
	jsr readChannelByte		// read the prearranged bitwise OR mask (ex: 00001000)

	

	bra .readLoop

.setEffectEchoOff	//244
	jsr readChannelByte		// read the prearranged bitwise AND mask (ex: 11110111)
...
.hackInEchoStuff:
// right, so we have a 1 or 0 in A,
// and it needs to get the right bit in the EON byte on or off depending on current channel,
//	EON
//	         7     6     5     4     3     2     1     0
//	      +-----+-----+-----+-----+-----+-----+-----+-----+
//	$4D   |VOIC7|VOIC6|VOIC5|VOIC4|VOIC3|VOIC2|VOIC1|VOIC0|
//	      +-----+-----+-----+-----+-----+-----+-----+-----+
// so say if D_CH0x says I'm in channel 0, and it's echo on, I have to
// take that EON that could be at 01010100 or whatever,
// and make it                    01010101
//	bonus: you can't assign stuff directly to DSP registers like it's a variable,
//		there's a whole step using I/O registers ADDR and DATA
//	ld* #{DSP_EON}
//	st* {ADDR}
//	ld* #%BBBBBBBB
//	st* {DATA}
	...
	ldy {D_CH0x}		// we're using Y to check channel numbers
	clc
	cmp #1				// comparing with the channel byte in A
	pha					// we're gonna mess with A to do bitwise ORs and ANDs
	bcc .noEcho			// if instrument number (A) is below 127, it's a non-echo instrument

	// echo
		lda channelMask,y	// channelMask: 00000001, 00000010, ... 10000000
		ora {D_EON}		// bitwise OR between current channel and DSP_EON to activate echo
		sta {D_EON}
		bra .finish

	.noEcho:
		lda channelRevMask,y 	// channelRevMask: 11111110, 11111101, ... 01111111
		and {D_EON}				// bitwise AND between current channel and DSP_EON to deactivate echo
		sta {D_EON}
	...
	.finish:
		...
		lda {D_EON}
		sta {DATA}
		pla // return A to how it was, Y takes care of itself in the parent routine
		rts
...
initDSPAndVars:
	...
	// !
	stx {D_EON}					//initialize the echo thingy
	// !
...
setInstrument:
	...
	// lol sorry Shiru-sensei here we goooo
	// jsr hackInEchoStuff
	// ~~~~~~~~~~~~~~~~
...
DSPInitData:
	...
	db {DSP_FLG}  ,%00000000				//no mute, disable echo
	db 0
