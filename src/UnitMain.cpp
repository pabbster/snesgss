...
unsigned char FLG_tracker = 0;

bool one_note = false;
bool one_note_noise = false;
bool one_note_echo = false;
...
int __fastcall TFormMain::ChannelCompile(songStruct *s,int chn,int start_row,int compile_adr,int &play_adr)
{
	...
	int current_note_code, offsetted_note_code, noise_note_code;
	current_note_code = 0;
	...
	for(row=0;row<length;++row)
	{
		...
		if(change)
		{
			...
			if(n->note>1)
			{
				...
				if ((noise_rows[row] & channel_or_mask[chn]) == 0){
					// we are playing a regular note, not noise
					...
					// we are playing noise.
					// we will use codes 173 to 203 (notes C3 to F#)
					// to play with the noise clock frequency instead of regular notes
					noise_note_code = current_note_code;
					if (noise_note_code < 173){ noise_note_code = 173; }
					if (noise_note_code > 203){ noise_note_code = 203; }
					if (noise_note_code != FLG_tracker){
						SPCChnMem[adr++] = 151; // code to modify FLG
						SPCChnMem[adr++] = noise_note_code - 172; // from 1 to 31
						FLG_tracker = noise_note_code;
					}
					SPCChnMem[adr++] = current_note_code; // also hit a note for all of that to take effect
				}
...
int __fastcall TFormMain::SongCompile(songStruct *s_original,int start_row,int start_adr,bool mute)
{
	...
	// Here I destroy Shiru's carefully measured efficiency
	// by just sending a "---" note instead of skipping muted/empty channels
	// so my tacked on stuff always applies to the intended channel
	// So in my version everything is always an 8-channel song
	// ... sorry lol
	for(chn=0;chn<8;++chn)
	{
...
bool __fastcall TFormMain::SPCCompile(songStruct *s,int start_row,bool mute,bool effects,int one_sample)
{
	...
	if (one_note){
		...
		use_EON = (one_note_echo) ? 0x01 : 0x00 ;
	}
	...
	temp_spc700_data[spc700_addr_NON] = use_NON;
	...
	temp_spc700_data[spc700_addr_EON] = use_EON;
...
void __fastcall TFormMain::SPCPlayNote(int note,int ins)
{
	...
	one_note = true;
	one_note_echo = ( insList[ins].echo_setting == 1 ) ;
	one_note_noise = ( insList[ins].noise_setting == 1) ;
	
	SPCPlaySong(&tempSong,0,false,ins);
	
	one_note = false;
	one_note_echo = false;
	one_note_noise = false;
}
...
void __fastcall TFormMain::FormCreate(TObject *Sender)
{
	...
	////////////// Open file with ParamStr(1) was here
